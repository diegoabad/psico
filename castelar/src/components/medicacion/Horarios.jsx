...
const Horarios = ({medicacion, setMedicacion, paciente}) => {
  ...
  const [values, setValues] = React.useState({medicacion:[], siete:[], ocho:[], doce:[], dieciseis:[], veinte:[],observaciones: '', miligramos: 0});
  ...
  if ((!values.medicacion.includes(estado.medicacion))){
    setValues({...values, medicacion: values.medicacion.concat(estado.medicacion), siete: values.siete.concat(estado.siete), ocho: values.ocho.concat(estado.ocho), doce: values.doce.concat(estado.doce), dieciseis: values.dieciseis.concat(estado.dieciseis), veinte: values.veinte.concat(estado.veinte)});
  ...
  <Paper className={classes.root} style = { {backgroundColor:'#1E90FF'} }>
    <Grid container spacing={3}>
      ...
      <Grid item xs={11} style ={{display:'flex', flexDirection:'column'}}>
        ...
        {estado.medicacion !== '' && ((estado.siete !== '' && estado.siete > 0) || (estado.ocho !=='' && estado.ocho > 0) || (estado.doce !== '' && estado.doce > 0 ) || ( estado.dieciseis !== '' && estado.dieciseis > 0 ) || ( estado.veinte !== '' && estado.veinte > 0 )) &&
