...
const addPsicologica = async (payload, id) => {
  try {
    ...
    await updateDoc(reference, { ultima_atencion: new Date() });
    return true;
  } catch (e) {
...
const EvolucionPsicologica = ({ setOpenFiliatorio, registro }) => {
  ...
  useEffect(() => {
    ...
    else setPsicologica({ ...psicologica, ...paciente[registro], fechaCreacion: paciente[registro].fechaCreacion.seconds ? new Date(paciente[registro].fechaCreacion.seconds * 1000) : new Date(paciente[registro].fechaCreacion) });
  }, []);
