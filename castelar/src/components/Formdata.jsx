...
const addFiliatorios = async (
	...
	try {
		...
		await setDoc(doc(busquedaRef, filiatorios.dni), {
			...
			activo: 'sí',
			dni: filiatorios.dni,
			ob_social: filiatorios.ob_social,
			edad: Math.trunc(
				(new Date() - filiatorios.fechaNacimiento) / (1000 * 60 * 60 * 24 * 365)
			),
			sexo: filiatorios.sexo,
			localidad: filiatorios.localidad,
			diagnostico: '',
			mod_atencion: '',
			inicio_tratamiento: '',
			psiquiatra: '',
...
const edit_Filiatorios = async (datos, id) => {
	try {
		...
		const reference = doc(db, 'busqueda', id);
		const upd = await updateDoc(ref, {
			nombre: datos.nombre,
			apellido: datos.apellido,
			nombreCompleto: `${datos.nombre} ${datos.apellido}`,
			historial: datos.historia_clinica,
			nro_afiliado: datos.nro_afiliado,
			activo: 'sí',
			dni: datos.dni,
			ob_social: datos.ob_social,
			edad: Math.trunc(
				(new Date() - datos.fechaNacimiento) / (1000 * 60 * 60 * 24 * 365)
			),
			sexo: datos.sexo,
