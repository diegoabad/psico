...
const EducacionFisica = ({ setOpenFiliatorio, registro }) => {
  ...
  const editar = useSelector((state) => state.editarFiliatorio)
    ...
    else setTerapias({...terapias, ...paciente[registro], fechaCreacion: paciente[registro].fechaCreacion.seconds? new Date(paciente[registro].fechaCreacion.seconds * 1000) : new Date(paciente[registro].fechaCreacion)});
	}, []);
