...
const ExamenClinico = ({ setOpenFiliatorio, registro }) => {
  ...
  const extraData = {
    ...
    else setIngreso({...ingreso, ...paciente[registro], fechaCreacion: paciente[registro].fechaCreacion.seconds? new Date(paciente[registro].fechaCreacion.seconds * 1000) : new Date(paciente[registro].fechaCreacion)});
	}, []);
