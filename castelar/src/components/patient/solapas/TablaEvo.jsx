	...
	return (
		<table>
			<tr className={s.trEsp}>
				...
				<th>Profesional</th>
				{solapa == 'Admisiones' ? <th className={s.widthEsp}>tipo</th> : null}
			...
			{evoluciones.length ? (
				evoluciones.map((evo, index) => {
					...
					return (
						<tr key={evo.fechaCreacion} className={s.trPro}>
							...
							{solapa == 'Admisiones' ? <td className={s.widthEsp}></td> : null}
				...
				<td className={s.sinEvo} colspan='4'>
					<div>
						{solapa == 'Admisiones'
							? 'No hay admisiones cargadas...'
							: 'No hay evoluciones . . .'}
					</div>
				</td>
			)}
